...
int main(int argc, char** argv) {
    ...
    if (input_filter_type == "LDF") {
        FilterVertices::LDFFilter(data_graph.get(), query_graph.get(), candidates, candidates_count);
    } else if (input_filter_type == "NLF") {
        FilterVertices::NLFFilter(data_graph.get(), query_graph.get(), candidates, candidates_count);
    } else if (input_filter_type == "GQL") {
        FilterVertices::GQLFilter(data_graph.get(), query_graph.get(), candidates, candidates_count);
    } else if (input_filter_type == "TSO") {
        FilterVertices::TSOFilter(data_graph.get(), query_graph.get(), candidates, candidates_count, tso_order, tso_tree);
    } else if (input_filter_type == "CFL") {
        FilterVertices::CFLFilter(data_graph.get(), query_graph.get(), candidates, candidates_count, cfl_order, cfl_tree);
    } else if (input_filter_type == "DPiso") {
        FilterVertices::DPisoFilter(data_graph.get(), query_graph.get(), candidates, candidates_count, dpiso_order, dpiso_tree);
    } else if (input_filter_type == "CECI") {
        FilterVertices::CECIFilter(data_graph.get(), query_graph.get(), candidates, candidates_count, ceci_order, ceci_tree, TE_Candidates, NTE_Candidates);
    }  else {
    ...
    if (input_filter_type != "CECI") {
        ...
        BuildTable::buildTables(data_graph.get(), query_graph.get(), candidates, candidates_count, edge_matrix);
    }
    ...
    if (input_filter_type != "CECI") {
        // memory_cost_in_bytes = BuildTable::computeMemoryCostInBytes(query_graph.get(), candidates_count, edge_matrix);
        // BuildTable::printTableCardinality(query_graph.get(), edge_matrix);
    }
    else {
        // memory_cost_in_bytes = BuildTable::computeMemoryCostInBytes(query_graph.get(), candidates_count, ceci_order, ceci_tree,
        //         TE_Candidates, NTE_Candidates);
        // BuildTable::printTableCardinality(query_graph.get(), ceci_tree, ceci_order, TE_Candidates, NTE_Candidates);
    }
    std::cout << "-----" << std::endl;
    ...
    if (input_order_type == "QSI") {
        GenerateQueryPlan::generateQSIQueryPlan(data_graph.get(), query_graph.get(), edge_matrix, matching_order, pivots);
    } else if (input_order_type == "GQL") {
        GenerateQueryPlan::generateGQLQueryPlan(data_graph.get(), query_graph.get(), candidates_count, matching_order, pivots);
    } else if (input_order_type == "TSO") {
        if (tso_tree == NULL) {
            GenerateFilteringPlan::generateTSOFilterPlan(data_graph.get(), query_graph.get(), tso_tree, tso_order);
        }
        GenerateQueryPlan::generateTSOQueryPlan(query_graph.get(), edge_matrix, matching_order, pivots, tso_tree, tso_order);
    } else if (input_order_type == "CFL") {
        if (cfl_tree == NULL) {
            int level_count;
            ui* level_offset;
            GenerateFilteringPlan::generateCFLFilterPlan(data_graph.get(), query_graph.get(), cfl_tree, cfl_order, level_count, level_offset);
            delete[] level_offset;
        }
        GenerateQueryPlan::generateCFLQueryPlan(data_graph.get(), query_graph.get(), edge_matrix, matching_order, pivots, cfl_tree, cfl_order, candidates_count);
    } else if (input_order_type == "DPiso") {
        if (dpiso_tree == NULL) {
            GenerateFilteringPlan::generateDPisoFilterPlan(data_graph.get(), query_graph.get(), dpiso_tree, dpiso_order);
        }

        GenerateQueryPlan::generateDSPisoQueryPlan(query_graph.get(), edge_matrix, matching_order, pivots, dpiso_tree, dpiso_order,
                                                    candidates_count, weight_array);
    }
    else if (input_order_type == "CECI") {
        GenerateQueryPlan::generateCECIQueryPlan(query_graph.get(), ceci_tree, ceci_order, matching_order, pivots);
    }
    else if (input_order_type == "RI") {
        GenerateQueryPlan::generateRIQueryPlan(data_graph.get(), query_graph.get(), matching_order, pivots);
    }
    else if (input_order_type == "VF2PP") {
        GenerateQueryPlan::generateVF2PPQueryPlan(data_graph.get(), query_graph.get(), matching_order, pivots);
    }
    else if (input_order_type == "Spectrum") {
        GenerateQueryPlan::generateOrderSpectrum(query_graph.get(), spectrum, order_num);
    }
    else {
    ...
    if (input_order_type != "Spectrum") {
        GenerateQueryPlan::checkQueryPlanCorrectness(query_graph.get(), matching_order, pivots);
        GenerateQueryPlan::printSimplifiedQueryPlan(query_graph.get(), matching_order);
    }
    else {
    ...
    if (input_csr_file_path.empty()) {
        ...
        suff::FilterManager::Init(root_dir, graph_name, create_filter);
    } else {
    ...
    suff::FilterManager::CreateFilters(data_graph, input_query_graph_file, query_graph, _matching_order);

    suff::FilterManager::LoadFilters();
    ...
    suff::FilterManager::PrepareFilters(data_graph, query_graph, _matching_order, _pivot, std::stoi(input_suff_k));
    ...
    if (input_engine_type == "EXPLORE") {
        embedding_count = EvaluateQuery::exploreGraph(data_graph.get(), query_graph.get(), edge_matrix, candidates,
                                                      candidates_count, matching_order, pivots, output_limit, call_count);
    } else if (input_engine_type == "LFTJ") {
        embedding_count = EvaluateQuery::LFTJ(data_graph.get(), query_graph.get(), edge_matrix, candidates, candidates_count,
                                              matching_order, output_limit, call_count);
    } else if (input_engine_type == "GQL") {
        embedding_count = EvaluateQuery::exploreGraphQLStyle(data_graph.get(), query_graph.get(), candidates, candidates_count,
                                                             matching_order, output_limit, call_count);
    } else if (input_engine_type == "QSI") {
        embedding_count = EvaluateQuery::exploreQuickSIStyle(data_graph.get(), query_graph.get(), candidates, candidates_count,
                                                             matching_order, pivots, output_limit, call_count);
    }
    else if (input_engine_type == "DPiso") {
        embedding_count = EvaluateQuery::exploreDPisoStyle(data_graph.get(), query_graph.get(), dpiso_tree,
...
//                                                           call_count);
        ...
        spectrum_analysis(data_graph.get(), query_graph.get(), edge_matrix, candidates, candidates_count, output_limit, spectrum, time_limit);
    }
    else if (input_engine_type == "CECI") {
        embedding_count = EvaluateQuery::exploreCECIStyle(data_graph.get(), query_graph.get(), ceci_tree, candidates, candidates_count, TE_Candidates,
