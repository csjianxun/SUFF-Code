...
static const long bit_count_table[256] = {0, 1, 1, 2, 1, 2, 2, 3, 1, 2, 2, 3, 2, 3, 3, 4,
...
public:
    ...
    virtual ~bloom_parameters() = default;
    ...
    virtual bool compute_optimal_parameters() {
        ...
        */
        ...
        // ensure table size is the power of 2, for fast index computing
        unsigned long long power_size = 1;
        while (power_size < optp.table_size) power_size <<= 1;
        optp.table_size = power_size;
...
public:
              ...
              desired_false_positive_probability_(p.false_positive_probability)
    {
        ...
        index_bits = table_size_ - 1;

        generate_unique_salt();

        bit_table_ = std::vector<std::atomic_uchar>(table_size_ / bits_per_char);
//        bit_table_ = std::vector<unsigned char>(table_size_ / bits_per_char);
        for (auto &block : bit_table_) {
            block = 0x00;
        }
    }

    bloom_filter(const bloom_filter& other): salt_count_(other.salt_count_), table_size_(other.table_size_),
        salt_(other.salt_), projected_element_count_(other.projected_element_count_), random_seed_(other.random_seed_),
        desired_false_positive_probability_(other.desired_false_positive_probability_), index_bits(other.index_bits),
//        bit_table_(std::vector<std::atomic<unsigned char>>(other.bit_table_.size())) {
        bit_table_(std::vector<std::atomic_uchar>(other.bit_table_.size())) {

        for (auto i = 0; i < bit_table_.size(); i++) {
            bit_table_[i] = 0x00;
            bit_table_[i] |= other.bit_table_[i];
        }

    }

    bloom_filter(bloom_filter&& other) = default;

    explicit bloom_filter(std::string &path)
            : salt_count_(0),
              table_size_(0),
              projected_element_count_(0),
              random_seed_(0),
              desired_false_positive_probability_(0.0)
    {
         ...
         */
        file.read((char *)&salt_count_, sizeof(unsigned int));
        salt_ = std::vector<bloom_type>(salt_count_);
        file.read((char *)&salt_[0], sizeof(bloom_type) * salt_count_);
        file.read((char *)&projected_element_count_, sizeof(unsigned long long int));
        file.read((char *)&random_seed_, sizeof(unsigned long long int));
        file.read((char *)&desired_false_positive_probability_, sizeof(double));
        file.read((char *)&table_size_, sizeof(unsigned long long int));
        index_bits = table_size_ - 1;
        bit_table_ = std::vector<std::atomic_uchar>(table_size_ / bits_per_char);
        file.read((char *)&bit_table_[0], sizeof(std::atomic_uchar) * table_size_ / bits_per_char);
        file.close();
    }
    ...
    void dump(const std::string &path) {
         ...
         * double                     desired_false_positive_probability_;
         */
        file.write((char *)&salt_count_, sizeof(unsigned int));
        file.write((char *)&salt_[0], sizeof(bloom_type) * salt_count_);
        file.write((char *)&projected_element_count_, sizeof(unsigned long long int));
        file.write((char *)&random_seed_, sizeof(unsigned long long int));
        file.write((char *)&desired_false_positive_probability_, sizeof(double));
        file.write((char *)&table_size_, sizeof(unsigned long long int));
        file.write((char *)&bit_table_[0], sizeof(std::atomic_uchar) * table_size_ / bits_per_char);
        file.close();
    }
...
//            }
            ...
            if ((bit_table_[bit_index >> 3] & bit_mask[bit]) == 0) {
                return false;
            }
        }
...
//    }
    ...
    inline unsigned long long int ones() const {
        // count the number of "one" bits
        unsigned long long int count = 0;
        for (auto &i : bit_table_) {
            count += bit_count_table[i];
        }
        return count;
    }
    ...
    inline double effective_fpp() const {
        ...
        */
        return std::pow(ones() / (double) size(), salt_.size());
//        return std::pow(1.0 - std::exp(-1.0 * salt_.size() * inserted_element_count_ / size()), 1.0 * salt_.size());
    }

    inline bloom_filter& operator = (const bloom_filter& other) {
        /* intersection */
        if (this != &other) {
            if (other.table_size_ != table_size_) {
                bit_table_ = std::vector<std::atomic_uchar>(other.bit_table_.size());
            }
            salt_count_ = other.salt_count_;
            table_size_ = other.table_size_;
            index_bits = other.index_bits;
            salt_ = other.salt_;
            projected_element_count_ = other.projected_element_count_;
            random_seed_ = other.random_seed_;
            desired_false_positive_probability_ =other.desired_false_positive_probability_;
            for (auto i = 0; i < bit_table_.size(); i++) {
                bit_table_[i] = 0x00;
                bit_table_[i] |= other.bit_table_[i];
            }
...
//        bit_index = hash % table_size_;
        bit_index = hash & index_bits;
//        bit_index = hash < table_size_ ? hash : hash % table_size_;
//        bit       = bit_index % bits_per_char;
    ...
    std::vector<bloom_type>    salt_;
    std::vector<std::atomic_uchar> bit_table_;
//    std::vector<unsigned char> bit_table_;
    unsigned int               salt_count_;
    unsigned long long int     table_size_;
    unsigned long long int     index_bits;
    unsigned long long int     projected_element_count_;
    unsigned long long int     random_seed_;
    double                     desired_false_positive_probability_;
};
