...
protected:
   ...
   typedef std::atomic_uchar cell_type;
   typedef std::vector<cell_type> table_type;
...
public:
     ...
     desired_false_positive_probability_(p.false_positive_probability)
   {
      ...
      bit_table_ = table_type(table_size_ / bits_per_char);
   ...
   inline bool operator == (const bloom_filter& f) const
   {
      if (this != &f)
      {
         // std::cout << "compare bf" << std::endl;
         // std::cout << salt_count_ << ", " << f.salt_count_ << std::endl;
         // std::cout << table_size_ << ", " << f.table_size_ << std::endl;
         // std::cout << bit_table_.size() << ", " << f.bit_table_.size() << std::endl;
         // std::cout << projected_element_count_ << ", " << f.projected_element_count_ << std::endl;
         // std::cout << inserted_element_count_ << ", " << f.inserted_element_count_ << std::endl;
         // std::cout << random_seed_ << ", " << f.random_seed_ << std::endl;
         // std::cout << desired_false_positive_probability_ << ", " << f.desired_false_positive_probability_ << std::endl;
         // std::cout << (salt_                               == f.salt_) << std::endl;
         // std::cout << (bit_table_                          == f.bit_table_) << std::endl;
         return
   ...
   inline bloom_filter& operator = (const bloom_filter& f)
   {
      if (this != &f)
      {
         salt_count_ = f.salt_count_;
         table_size_ = f.table_size_;
         if (f.bit_table_.size() != bit_table_.size()) {
               bit_table_ = std::vector<cell_type>(f.bit_table_.size());
         }
         for (auto i = 0; i < bit_table_.size(); i++) {
            bit_table_[i] = f.bit_table_[i].load();
         }
         // bit_table_  = f.bit_table_;
         salt_       = f.salt_;
   ...
   explicit bloom_filter(std::string &path)
              ...
              desired_false_positive_probability_(0.0)
    {
        ...
        file.read((char *)&inserted_element_count_, sizeof(unsigned long long int));
        ...
        std::size_t bit_table_size;
        file.read((char *)&bit_table_size, sizeof(std::size_t));
        bit_table_ = table_type(bit_table_size);
        file.read((char *)(&bit_table_[0]), sizeof(cell_type) * bit_table_size);
        file.close();
    }
   ...
   void dump(const std::string &path) {
        ...
        file.write((char *)&inserted_element_count_, sizeof(unsigned long long int));
        ...
        auto bit_table_size = bit_table_.size();
        file.write((char *)&bit_table_size, sizeof(std::size_t));
        file.write((char *)(&bit_table_[0]), sizeof(cell_type) * bit_table_size);
        file.close();
    }
...
public:
   ...
   inline bool compress(const double& percentage)
   {
      ...
      for (std::size_t i = 0; i < new_tbl_raw_size; i++) {
         tmp[i] = bit_table_[i].load();
      }
      ...
      bit_table_ = table_type(tmp.size());
      for (std::size_t i = 0; i < tmp.size(); i++) {
         bit_table_[i] = tmp[i].load();
      }
